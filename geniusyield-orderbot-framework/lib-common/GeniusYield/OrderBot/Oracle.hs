...
import Network.HTTP.Req (req, responseBody, runReq, defaultHttpConfig, https, http, port, (/:), ReqBodyJson(..), POST(..), jsonResponse)
...
instance Aeson.ToJSON OracleCertificateRequest where
  toJSON OracleCertificateRequest {..} = Aeson.object
...
parseAssetClass (policy, token) =
    ...
    where eitherToMaybe (Left _)  = Nothing
...
getOracleCertificate (baseAsset, quoteAsset) = do
  let (baseSymbol, baseToken)   = renderAssetClass baseAsset
      ...
      requestBody = OracleCertificateRequest
